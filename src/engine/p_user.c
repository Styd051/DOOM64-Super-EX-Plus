    ...
    if (cmd->buttons & BT_CHANGE) {
        if (!(cmd->buttons2 & (BT2_NEXTWEAP | BT2_PREVWEAP))) {
            ...
            if (newweapon == wp_supershotgun
                && player->weaponowned[wp_quadshotgun]
                && player->readyweapon != wp_quadshotgun) {
                newweapon = wp_quadshotgun;
            }

            if (newweapon == wp_quadshotgun
                && player->weaponowned[wp_hexashotgun]
                && player->readyweapon != wp_hexashotgun) {
                newweapon = wp_hexashotgun;
            }
            ...
            if (newweapon == wp_plasma
                && player->weaponowned[wp_flamethrower]
                && player->readyweapon != wp_flamethrower) {
                newweapon = wp_flamethrower;
            }

            if (newweapon == wp_bfg 
                && player->weaponowned[wp_bfg10k]
                && player->readyweapon != wp_bfg10k) {
                newweapon = wp_bfg10k;
            }
